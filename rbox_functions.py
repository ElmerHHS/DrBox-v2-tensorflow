...
DecodeAndNMS.argtypes = (
POINTER(c_double), POINTER(c_double), POINTER(c_int), POINTER(c_double), POINTER(c_int), c_double)
...
def MatchRBox(priorbox, groundtruth, overlap_threshold, is180):
    ...
    for i in range(len(priorbox)):
        for j in range(len(groundtruth)):
            ...
            if is180:
                overlaps[i, j] = overlap(cpriorbox, cgroundtruth) * abs(
                    math.cos((priorbox[i][4] - groundtruth[j][4]) / 180 * math.pi))
            else:
                overlaps[i, j] = overlap(cpriorbox, cgroundtruth) * math.cos(
                    (priorbox[i][4] - groundtruth[j][4]) / 180 * math.pi)
...
def NMSOutput(rboxlist, scorelist, nms_threshold, label, test_rbox_output_path):
    ...
    with open(test_rbox_output_path, 'w') as fid:
        for i in range(num_preds.value):
            index_i = indices_c[i]
            fid.write(
                '{} {} {} {} {} {} {}\n'.format(loc_c[5 * index_i], loc_c[5 * index_i + 1], loc_c[5 * index_i + 2],
                                                loc_c[5 * index_i + 3], label,
