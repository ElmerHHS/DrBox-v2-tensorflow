...
PRIOR_HEIGHTS = [[20.0, 24.0, 28.0, 32.0], [28.0, 34.0, 40.0, 48.0, 56.0]]  # Broodjes
# PRIOR_WIDTHS = [[15.0, 25.0, 35.0, 45.0], [20.0, 35.0, 50.0, 80.0, 100.0]]
PRIOR_WIDTHS = [[50.0, 60.0, 70.0, 80.0], [70.0, 85.0, 100.0, 120.0, 140.0]]  # Broodjes
...
class DrBoxNet():
    def __init__(self):
        ...
        self.total_prior_num = FEA_HEIGHT4 * FEA_WIDTH4 * self.prior_num[1] + FEA_HEIGHT3 * FEA_WIDTH3 * self.prior_num[
            0] * USE_THIRD_LAYER
        self.para_num = 5
        ...
        if FLAGS.train:
            self.detector = VGG16(self.prior_num, self.para_num, self.cls_num, FPN_NET, USE_THIRD_LAYER,
                                  TRAIN_BATCH_SIZE)
        else:
            self.detector = VGG16(self.prior_num, self.para_num, self.cls_num, FPN_NET, USE_THIRD_LAYER,
                                  TEST_BATCH_SIZE)
    ...
    def compute_conf_loss(self):
        ...
        if USE_FOCAL_LOSS:
            ...
            self.conf_pos_loss = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(logits=pos_tensor, labels=self.pos_label))
            self.conf_neg_loss = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(logits=neg_tensor, labels=self.neg_label) * self.batch_neg_mask)
    ...
    def get_encoded_positive_box(self):
        prior_box4 = PriorRBox(IM_HEIGHT, IM_WIDTH, FEA_HEIGHT4, FEA_WIDTH4, STEPSIZE4, PRIOR_ANGLES, PRIOR_HEIGHTS[1],
                               PRIOR_WIDTHS[1])
        prior_box3 = PriorRBox(IM_HEIGHT, IM_WIDTH, FEA_HEIGHT3, FEA_WIDTH3, STEPSIZE3, PRIOR_ANGLES, PRIOR_HEIGHTS[0],
                               PRIOR_WIDTHS[0])
        if USE_THIRD_LAYER:
        ...
        for k in range(self.train_im_num):
            ...
            if not LOAD_PREVIOUS_POS:
                self.ind_one_hot[idx], self.positive_indice[idx], self.encodedbox[idx] = MatchRBox(prior_box, gt_box,
                                                                                                   OVERLAP_THRESHOLD,
                                                                                                   IS180)
        ...
        for iter_num in range(ITERATION_NUM + 1):
            ...
            for batch_idx in batch_list:
                ...
                if k == 0:
                    ...
                    batch_pos_idx = np.concatenate(
                        (batch_pos_idx, self.positive_indice[real_idx] + k * self.total_prior_num), axis=0)
            ...
            self.sess.run(train_step, feed_dict={self.input_im: input_im, self.batch_pos_box: batch_pos_box,
                                                 self.batch_pos_ind: batch_pos_ind,
                                                 self.batch_pos_idx: batch_pos_idx, self.batch_neg_mask: batch_neg_mask,
            ...
            if counter % DISPLAY_INTERVAL == 0:
                loss, loc_loss, conf_loss, conf_pos_loss, conf_neg_loss, reg_loss = self.sess.run([
                    ...
                    feed_dict={self.input_im: input_im,
                               self.batch_pos_box: batch_pos_box, self.batch_pos_ind: batch_pos_ind,
                               self.batch_pos_idx: batch_pos_idx, self.batch_neg_mask: batch_neg_mask,
                               self.pos_label: pos_label, self.neg_label: neg_label})
                with open(SAVE_PATH + '/loss.txt', 'ab+') as files:
                    files.write((
                                    "counter:[%2d], loss:%.8f, loc_loss:%.8f, conf_loss:%.8f, conf_pos_loss:%.8f, conf_neg_loss:%.8f, reg_loss:%.8f") % (
                                counter, loss, loc_loss, conf_loss, conf_pos_loss, conf_neg_loss, reg_loss))
                    files.write('\n')
                print(
                    "counter:[%2d], loss:%.8f, loc_loss:%.8f, conf_loss:%.8f, conf_pos_loss:%.8f, conf_neg_loss:%.8f, reg_loss:%.8f") % (
    ...
    def test(self):
        ...
        for test_info in self.test_im_list:
            ...
            for i in range(len(TEST_RESOLUTION_OUT)):
                ...
                while 1:
                    if islast == 0:
                        width_S = IM_WIDTH * TEST_RESOLUTION_OUT[
                            i] / TEST_RESOLUTION_IN  # int(round(IM_WIDTH * TEST_RESOLUTION_OUT[i] / TEST_RESOLUTION_IN))
                        height_S = IM_HEIGHT * TEST_RESOLUTION_OUT[
                            i] / TEST_RESOLUTION_IN  # int(round(IM_HEIGHT * TEST_RESOLUTION_OUT[i] / TEST_RESOLUTION_IN))
                        xEnd = xBegin + width_S
                        ...
                        subimage[0:yEndHat - yBeginHat, 0:xEndHat - xBeginHat, :] = image_i[yBeginHat:yEndHat,
                                                                                    xBeginHat:xEndHat, :]
                    ...
                    if count == TEST_BATCH_SIZE or islast == 1:
                        loc_preds, conf_preds = self.sess.run([self.loc, self.conf_softmax],
                                                              feed_dict={self.input_im: inputdata})
                        for j in range(TEST_BATCH_SIZE):
                            ...
                            if len(loc_preds_j) > 0:
                                rbox, score = DecodeNMS(loc_preds_j, prior_boxes_j, conf_preds_j, inputloc_j, index,
                                                        TEST_NMS_THRESHOLD, IM_HEIGHT, IM_WIDTH)
